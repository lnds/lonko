// lonko focus <n>
// Switches the tmux client to the Nth Claude session tracked by lonko.
...
pub fn run(n: usize) -> anyhow::Result<()> {
    if n == 0 {
        tmux_msg("lonko: position must be 1-9");
    ...
    let content = match std::fs::read_to_string(&path) {
        ...
        _ => {
            tmux_msg("lonko: not running or no sessions");
            anyhow::bail!("lonko sessions cache not available");
        }
    };

    let line = content.lines().nth(n - 1).map(str::trim).unwrap_or("");
    let pane_id = if line.is_empty() {
        tmux_msg(&format!("lonko: session {n} not ready yet"));
    ...
    match (switch, select) {
        ...
        _ => {
            tmux_msg(&format!("lonko: session {n} pane gone"));
            anyhow::bail!("tmux switch-client failed for pane {pane_id}");
        }
    }
}

/// Path where lonko writes the ordered session list.
