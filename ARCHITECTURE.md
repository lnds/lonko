...
Lonko is a TUI that monitors Claude Code sessions running in tmux.
...
```
crates/
  lonko/          Main TUI binary
  lonko-hook/     Lightweight forwarder (stdin -> Unix socket)
scripts/
  lonko-panel.sh  Toggle side panel (installed to ~/.config/tmux/scripts/)
  lonko-follow.sh Auto-follow between tmux windows (same)
```
...
src/
  ...
  focus.rs           `lonko focus N` subcommand
...
enriches it with `$TMUX_PANE`, and forwards it to lonko via Unix
socket. If lonko is not running and the event is `Notification` or
...
1. **Hook events** -- Claude Code fires hooks (SessionStart,
   UserPromptSubmit, PreToolUse, etc.) that `lonko-hook` forwards
...
2. **Lifecycle files** -- A filesystem watcher monitors
   ...
   that started before lonko.

3. **tmux scanner** -- Every 5 seconds, lonko scans all tmux panes
...
`notification_type: "permission_prompt"`. Lonko sets the session
status to `WaitingForUser` and the footer shows `y:yes w:always n:no`.

When the user presses `y`/`w`/`n`, lonko finds the first
...
Lonko and the tmux scripts communicate through files in the OS
...
1. **Binaries**: `cargo install` for `lonko` and `lonko-hook`
   to `~/.cargo/bin/`
2. **tmux scripts**: copies `scripts/*.sh` to `~/.config/tmux/scripts/`
3. **tmux config fragment**: writes `~/.config/tmux/lonko.conf` with
   all hooks, keybindings, and escape-sequence routing lonko needs
4. **Claude hooks**: runs `lonko --install-hooks` to configure
   `~/.claude/settings.json`

## tmux integration

After running `install.sh`, lonko generates
`~/.config/tmux/lonko.conf` with everything you need to wire it
...
line in your config, so lonko's `-ga` (append) stacks correctly on
...
lonko automatically picks up new bindings. Do not edit it by
hand — customizations would be lost on the next install.

### What the generated file contains

**Auto-follow hooks** — lonko re-parents to the active window when
...
**Panel toggle** — `prefix + s` toggles the lonko side panel:
...
sends `\e[sa` / `\e[ss`, lonko opens the Agents or Sessions tab:
...
`\e[sy` / `\e[sn` / `\e[sw`, lonko responds to the current
...
The `lonko respond` subcommand talks to the running lonko over
its Unix socket (same channel `lonko-hook` uses), so this works
...
use `lonko focus N` in your shortcut system. For example, in a
tmux keybinding:

```tmux
bind 1 run-shell "lonko focus 1"
bind 2 run-shell "lonko focus 2"
# ... etc
```

Lonko writes the session list to the cache file
`lonko-sessions-info` (tab-separated: number, project name, cwd)
